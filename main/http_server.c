...
static esp_err_t h_root_get(httpd_req_t *r)
{
    const char *html =
        ...
        "<title>rftool</title>"
        "<style>"
        "body{font-family:sans-serif;max-width:720px;margin:0 auto;padding:8px;"
        "overflow-y:auto;-webkit-overflow-scrolling:touch}"
        "h1{color:#2a4;font-size:1.4em}"
        "button{padding:10px 16px;margin:4px 2px;background:#2a4;color:#fff;border:0;"
        "border-radius:6px;cursor:pointer;font-size:14px;min-height:36px;touch-action:manipulation}"
        "button:active{background:#183}"
        "input,select{padding:6px;margin:2px;border:1px solid #bbb;border-radius:4px;font-size:14px}"
        "label{display:inline-block;margin:2px 4px}"
        ".sec{margin:8px 0;padding:10px;border:1px solid #ddd;border-radius:8px}"
        "#status{background:#f4f8f4;padding:8px;border-radius:6px;"
        ...
        "<label>ch<input id='ch' type='number' min='1' max='14' value='6' style='width:40px'></label>"
        "<label>n<input id='cnt' type='number' min='0' value='0' style='width:50px' placeholder='0=forever'></label>"
        "<button onclick='doSniff()'>Sniff</button><br>"
        "<label>dwell ms<input id='dwell' type='number' min='100' value='1000' style='width:60px'></label>"
        "<button onclick='doSniffAuto()'>Auto</button>"
        "<button onclick='doStop()'>Stop</button>"
        "</div>"
        "<div class='sec'><b>Deauth</b> (auth only)<br>"
        "<label>BSSID<input id='bssid' placeholder='aa:bb:cc:dd:ee:ff' style='width:140px'></label>"
        "<label>STA<input id='sta' placeholder='empty=bcast' style='width:120px'></label><br>"
        "<label>n<input id='dcnt' type='number' min='0' value='20' style='width:50px'></label>"
        "<label>ms<input id='dint' type='number' min='0' value='0' style='width:50px'></label>"
        "<button onclick='doDeauth()'>Start</button>"
        "</div>"
        "<div class='sec'><b>Flood</b> (auth only)<br>"
        "<label>prefix<input id='prefix' value='FREE-WIFI-' style='width:100px'></label>"
        "<label>n<input id='bcnt' type='number' min='0' value='100' style='width:50px'></label>"
        "<button onclick='doBeacon()'>Beacon</button><br>"
        "<label>n<input id='pcnt' type='number' min='0' value='200' style='width:50px'></label>"
        ...
        "async function api(u,b){var o={headers:{'Content-Type':'application/json'}};"
        "if(b){o.method='POST';o.body=JSON.stringify(b)}"
        "var r=await fetch(u,o);return await r.json()}"
        "async function refresh(){try{var s=await api('/api/status');"
        "$('s').textContent=JSON.stringify(s,null,2)}catch(e){}}"
        ...
        "if(d.aps)for(var i=0;i<d.aps.length;i++){var a=d.aps[i];"
        ...
        "if(d.clients)for(var i=0;i<d.clients.length;i++){var c=d.clients[i];"
