...
static esp_err_t h_root_get(httpd_req_t *r)
{
    const char *html =
        ...
        "<title>rftool - 控制面板</title>"
        "<style>body{font-family:sans-serif;max-width:720px;margin:20px auto;padding:0 12px}"
        "h1{color:#2a4}button{padding:8px 14px;margin:4px;background:#2a4;color:#fff;border:0;border-radius:4px;cursor:pointer}"
        "button:hover{background:#183}input,select{padding:6px;margin:2px;border:1px solid #bbb;border-radius:4px}"
        ".sec{margin:16px 0;padding:12px;border:1px solid #ddd;border-radius:8px}"
        "#status{background:#f4f8f4;padding:10px;border-radius:6px;font-family:monospace;white-space:pre-wrap}"
        "table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:6px 8px;text-align:left;font-size:13px}"
        "th{background:#f0f4f0}</style></head><body>"
        "<h1>ESP32-C5 rftool 控制面板</h1>"
        "<div id='status'>加载中...</div>"
        "<div class='sec'><h3>嗅探控制</h3>"
        "<label>信道 <input id='ch' type='number' min='1' max='14' value='6'></label>"
        "<label>包数 <input id='cnt' type='number' min='0' value='0' placeholder='0=永久'></label>"
        "<button onclick='doSniff()'>单信道嗅探</button>"
        "<label>停留ms <input id='dwell' type='number' min='100' value='1000'></label>"
        "<button onclick='doSniffAuto()'>自动轮询</button>"
        "<button onclick='doStop()'>停止</button>"
        "</div>"
        "<div class='sec'><h3>Deauth 注入 (授权使用)</h3>"
        "<label>BSSID <input id='bssid' placeholder='aa:bb:cc:dd:ee:ff'></label>"
        "<label>STA <input id='sta' placeholder='cc:dd:... 空=广播'></label>"
        "<label>数量 <input id='dcnt' type='number' min='0' value='20'></label>"
        "<label>间隔ms <input id='dint' type='number' min='0' value='0'></label>"
        "<button onclick='doDeauth()'>启动 Deauth</button>"
        "</div>"
        "<div class='sec'><h3>Beacon / Probe Flood (授权使用)</h3>"
        "<label>SSID前缀 <input id='prefix' value='FREE-WIFI-'></label>"
        "<label>数量 <input id='bcnt' type='number' min='0' value='100'></label>"
        "<button onclick='doBeacon()'>Beacon Flood</button>"
        "<label>数量 <input id='pcnt' type='number' min='0' value='200'></label>"
        "<button onclick='doProbe()'>Probe Flood</button>"
        "<button onclick='doStop()'>停止注入</button>"
        "</div>"
        "<div class='sec'><h3>数据库 <button onclick='loadAll()'>刷新</button></h3>"
        "<h4>APs</h4><table id='aptbl'><thead><tr><th>BSSID</th><th>SSID</th><th>Ch</th><th>RSSI</th><th>加密</th><th>厂商</th><th>PMF</th></tr></thead><tbody></tbody></table>"
        "<h4>Clients</h4><table id='cltbl'><thead><tr><th>MAC</th><th>BSSID</th><th>RSSI</th><th>包</th><th>厂商</th></tr></thead><tbody></tbody></table>"
        "</div>"
        "<div class='sec'><h3>导出</h3>"
        "<a href='/api/export/json' target='_blank'><button>导出 JSON</button></a>"
        "<a href='/api/export/csv' target='_blank'><button>导出 CSV</button></a>"
        "</div>"
        "<script>"
        "async function j(url,body){const o={headers:{'Content-Type':'application/json'}};if(body){o.method='POST';o.body=JSON.stringify(body)};return await(await fetch(url,o)).json()}"
        "async function refresh(){try{const s=await j('/api/status');document.getElementById('status').textContent=JSON.stringify(s,null,2)}catch(e){}}"
        "async function loadAll(){await refresh();try{const a=await(await fetch('/api/db/aps')).json();const t=document.querySelector('#aptbl tbody');t.innerHTML='';a.forEach(e=>{t.innerHTML+=<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>})}catch(e){}"
        "try{const c=await(await fetch('/api/db/clients')).json();const t=document.querySelector('#cltbl tbody');t.innerHTML='';c.forEach(e=>{t.innerHTML+=<tr><td></td><td></td><td></td><td></td><td></td></tr>})}catch(e){}}"
        "function doSniff(){j('/api/sniff/start',{channel:parseInt(document.getElementById('ch').value),count:parseInt(document.getElementById('cnt').value)}).then(refresh)}"
        "function doSniffAuto(){j('/api/sniff/auto',{dwell_ms:parseInt(document.getElementById('dwell').value)}).then(refresh)}"
        "function doStop(){j('/api/stop',{}).then(refresh)}"
        "function doDeauth(){const body={bssid:document.getElementById('bssid').value,station:document.getElementById('sta').value,count:parseInt(document.getElementById('dcnt').value),interval_ms:parseInt(document.getElementById('dint').value),reason:7};j('/api/deauth/start',body).then(refresh)}"
        "function doBeacon(){j('/api/beaconflood/start',{prefix:document.getElementById('prefix').value,count:parseInt(document.getElementById('bcnt').value),interval_ms:0}).then(refresh)}"
        "function doProbe(){j('/api/probeflood/start',{count:parseInt(document.getElementById('pcnt').value),interval_ms:0}).then(refresh)}"
        "setInterval(refresh,2000);refresh();loadAll();"
        "</script></body></html>";
